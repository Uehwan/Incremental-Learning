## Fusion ART and Fusion ARTMAP
## mostly borrowed from https://github.com/AIOpenLab/art

## dev. history
# 18.11.08 corrected choice-by-difference
...
#        print(x.shape)
        x = np.atleast_2d(x)
        Fusion_ws = np.minimum(x, self.w)
        Fusion_norm = l1_norm(Fusion_ws)
        scores = Fusion_norm / (self.gamma + l1_norm(self.w))
        threshold = Fusion_norm / l1_norm(x) >= self.rho
    ...
    def test(self, x):
        ...
        for i, sample in enumerate(samples):
            categories[i] = self._match_category(np.atleast_2d(sample))
        return categories


class FusionARTMAP(object):
    """
    Fusion ARTMAP

    A supervised version of FusionART
    """

    def __init__(self, alpha=1.0, gamma=0.01, rho=0.5, epsilon=0.0001, complement_coding=True):
        """
        :param alpha: learning rate [0,1]
        :param gamma: regularization term >0
        :param rho: rho [0,1]
    ...
    def _match_category(self, x, y=None):
        ...
        Fusion_ws = np.minimum(x, self.w)
        Fusion_norm = l1_norm(Fusion_ws)
        scores = Fusion_norm + (1 - self.gamma) * (l1_norm(x) - l1_norm(self.w))
        norms = Fusion_norm / l1_norm(x)
...
if __name__ == '__main__':
    ...
    net = FusionART(alpha=0.5, rho=0.75)
